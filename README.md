...
Current debug build marker: **b057**.
...
- Refuel route hardening: water/coal/diesel route targets now skip tracks without routeable IDs, and route walking no longer throws a null dictionary-key error on non-junction route sections
...
- Non-self-lapping brake notch fix: steam/DM3-style pulse braking now snaps out of the Hold band so brake commands are Release or Apply/Emergency instead of drifting through Hold
- Steam light-engine brake fix: when a steam loco is running alone, AI brake commands now use the independent brake and release the train brake, matching the diesel light-engine path
- Steam tender light-engine fix: steam locos coupled only to tender cars are still treated as light engine for independent-brake AI control
- Turnout speed lookahead: route transitions through junctions now add a conservative 30 km/h turnout cap into the AI lookahead, so it starts slowing before reaching a switch instead of relying on the old 60 km/h junction-end cap
...
- Destination siding clearance: after the route hits finish, the AI keeps rolling at 10 km/h until the tracked rear end of the train, plus a 25 m buffer, has entered the destination track before it performs the final stop
- Destination siding clearance hardening: final stop now also requires every train car's current logic track to be the destination track, with a larger tail buffer to cover bogie/coupler overhang
...
- Reload into **b057** and confirm the Comms Radio build marker updates after UMM reload
...
- Test S060/S282 non-self-lapping brake notches: during overspeed correction, confirm it moves to Apply rather than sitting in Hold with empty brake cylinders
- Test steam light-engine braking: with no cars attached, confirm the AI uses independent brake rather than train brake for speed correction and stopping
- Test steam with tender only: confirm the AI still uses independent brake and does not treat the tender as a hauled consist
- Test light-engine turnout approach: confirm it targets about 25 km/h before switch transitions; if it still enters fast, the remaining issue is braking distance/deceleration rather than turnout detection
...
- Re-test destination siding clearance after b053: confirm every car is on the destination/siding track before the AI performs the final stop
